...
Shader "LazyEti/URP/FakePointLight"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float2 WorldToScreen83_g2093( float3 pos )
			{
			...
			PackedVaryings VertexFunction( Attributes input  )
			{
				...
				float Dist41_g2085 = distance( ( POSITION653 * _Vector0 ) , ( _Vector0 * _WorldSpaceCameraPos ) );
				float vertexToFrag49_g2085 = ( saturate( ( 1.0 - ( ( Dist41_g2085 - _FarFade ) / _FarTransition ) ) ) * saturate( ( ( Dist41_g2085 - _CloseFade ) / _CloseTransition ) ) );
				output.ase_texcoord6.w = vertexToFrag49_g2085;
				float dotResult3_g2084 = dot( -_MainLightPosition.xyz , float3( 0,1,0 ) );
				float vertexToFrag9_g2084 = saturate( ( dotResult3_g2084 * 4.0 ) );
				output.ase_texcoord7.x = vertexToFrag9_g2084;
				#ifdef _PARTICLEMODE_ON
				...
				#endif
				float vertexToFrag32_g2093 = ( ( _HaloSize * ( FlickerSize477 * staticSwitch4535 ) ) * 0.5 );
				output.ase_texcoord7.y = vertexToFrag32_g2093;
				float3 pos75_g2093 = POSITION653;
				float vertexToFrag15_g2096 = ( unity_OrthoParams.w == 0.0 ? ( distance( _WorldSpaceCameraPos , pos75_g2093 ) / -UNITY_MATRIX_P[ 1 ][ 1 ] ) : unity_OrthoParams.y );
				output.ase_texcoord7.z = vertexToFrag15_g2096;
				float3 ase_positionWS = TransformObjectToWorld( ( input.positionOS ).xyz );
				float3 ase_viewVectorWS = ( _WorldSpaceCameraPos.xyz - ase_positionWS );
				float3 ase_viewDirSafeWS = SafeNormalize( ase_viewVectorWS );
				float dotResult31_g2093 = dot( ase_viewDirSafeWS , ( _WorldSpaceCameraPos - pos75_g2093 ) );
				float vertexToFrag36_g2093 = step( 0.0 , dotResult31_g2093 );
				output.ase_texcoord7.w = vertexToFrag36_g2093;
				float vertexToFrag62_g2093 = distance( _WorldSpaceCameraPos , pos75_g2093 );
				output.ase_texcoord8.x = vertexToFrag62_g2093;
			...
			half4 frag ( PackedVaryings input  ) : SV_Target
			{
				...
				float depthLinearEye6_g2080 = LinearEyeDepth( SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_positionSSNorm.xy ), _ZBufferParams );
				float4x4 invertVal5_g2081 = Inverse4x4( unity_ObjectToWorld );
				float dotResult4_g2080 = dot( WorldViewDirection , -mul( unity_ObjectToWorld, float4( (transpose( mul( invertVal5_g2081, UNITY_MATRIX_I_V ) )[2]).xyz , 0.0 ) ).xyz );
				float2 UV22_g2083 = ase_positionSSNorm.xy;
				float2 localUnStereo22_g2083 = UnStereo( UV22_g2083 );
				float2 break64_g2082 = localUnStereo22_g2083;
				float depth01_69_g2082 = SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_positionSSNorm.xy );
				#ifdef UNITY_REVERSED_Z
				float staticSwitch38_g2082 = ( 1.0 - depth01_69_g2082 );
				#else
				float staticSwitch38_g2082 = depth01_69_g2082;
				#endif
				...
				float3 ReconstructedPos539 = (( unity_OrthoParams.w == 0.0 ? float4( ( ( depthLinearEye6_g2080 * ( WorldViewDirection / dotResult4_g2080 ) ) + _WorldSpaceCameraPos ) , 0.0 ) : mul( unity_CameraToWorld, appendResult49_g2082 ) )).xyz;
				float3 ase_objectPosition = GetAbsolutePositionWS( UNITY_MATRIX_M._m03_m13_m23 );
				#ifdef _PARTICLEMODE_ON
				...
				float3 UVS73_g2086 = ( ( ReconstructedPos539 * 0.1 ) * _NoiseScale );
				...
				float3 temp_cast_4 = (3.0).xxx;
				float3 temp_output_141_0_g2086 = pow( abs( worldNormals1927 ) , temp_cast_4 );
				float3 temp_cast_5 = (1.0).xxx;
				float dotResult144_g2086 = dot( temp_output_141_0_g2086 , temp_cast_5 );
				float3 break147_g2086 = ( saturate( temp_output_141_0_g2086 ) / dotResult144_g2086 );
				float4 ase_positionSS_Pixel = ASEScreenPositionNormalizedToPixel( ase_positionSSNorm );
				float dither151_g2086 = Dither8x8Bayer( fmod( ase_positionSS_Pixel.x, 8 ), fmod( ase_positionSS_Pixel.y, 8 ) );
				float2 lerpResult19_g2086 = lerp( (UVS73_g2086).xz , ( (UVS73_g2086).yz * 0.9 ) , round( ( ( ( ( 1.0 - break147_g2086.x ) * break147_g2086.x ) * dither151_g2086 ) + break147_g2086.x ) ));
				float2 lerpResult20_g2086 = lerp( lerpResult19_g2086 , ( (UVS73_g2086).xy * 0.94 ) , round( ( break147_g2086.z + ( dither151_g2086 * ( break147_g2086.z * ( 1.0 - break147_g2086.z ) ) ) ) ));
				float temp_output_60_0_g2086 = RANDOMNESS3727;
				float mulTime41_g2086 = _TimeParameters.x * ( ( _NoiseMovement + ( temp_output_60_0_g2086 * 0.1 ) ) * 0.2 );
				float Time70_g2086 = ( mulTime41_g2086 + ( temp_output_60_0_g2086 * PI ) );
				float4 tex2DNode3_g2086 = SAMPLE_TEXTURE2D( _NoiseTexture, sampler_NoiseTexture, ( lerpResult20_g2086 + ( Time70_g2086 * float2( 1.02,0.87 ) ) ) );
				#if defined( _TEXTUREPACKING_RED )
				float staticSwitch211_g2086 = tex2DNode3_g2086.r;
				#elif defined( _TEXTUREPACKING_REDXGREEN )
				float staticSwitch211_g2086 = tex2DNode3_g2086.g;
				#elif defined( _TEXTUREPACKING_ALPHA )
				float staticSwitch211_g2086 = tex2DNode3_g2086.a;
				#else
				float staticSwitch211_g2086 = tex2DNode3_g2086.r;
				#endif
				float4 tex2DNode14_g2086 = SAMPLE_TEXTURE2D( _NoiseTexture, sampler_NoiseTexture, ( ( lerpResult20_g2086 * 0.7 ) + ( Time70_g2086 * float2( -0.72,-0.67 ) ) ) );
				#if defined( _TEXTUREPACKING_RED )
				float staticSwitch212_g2086 = tex2DNode14_g2086.r;
				#elif defined( _TEXTUREPACKING_REDXGREEN )
				float staticSwitch212_g2086 = tex2DNode14_g2086.r;
				#elif defined( _TEXTUREPACKING_ALPHA )
				float staticSwitch212_g2086 = tex2DNode14_g2086.a;
				#else
				float staticSwitch212_g2086 = tex2DNode14_g2086.r;
				#endif
				#ifdef ___NOISE____ON
				float staticSwitch186_g2086 = ( 1.0 + ( ( staticSwitch211_g2086 * staticSwitch212_g2086 * _Noisiness ) - ( _Noisiness * 0.2 ) ) );
				#else
				float staticSwitch186_g2086 = 1.0;
				#endif
				float noise3969 = staticSwitch186_g2086;
				...
				float temp_output_8_0_g2091 = lPosterize4215;
				float BrightnessBoost3902 = saturate( pow( temp_output_3981_0 , 30.0 ) );
				float temp_output_9_0_g2091 = ( smoothstepResult745 + BrightnessBoost3902 );
				float temp_output_5_0_g2091 = ( 256.0 / temp_output_8_0_g2091 );
				float GradientMask555 = ( smoothstepResult745 * ( temp_output_8_0_g2091 <= 0.0 ? temp_output_9_0_g2091 : saturate( ( floor( ( temp_output_9_0_g2091 * temp_output_5_0_g2091 ) ) / temp_output_5_0_g2091 ) ) ) );
				float2 temp_cast_6 = (( 1.0 - GradientMask555 )).xx;
				float4 temp_output_2_0_g2092 = ( SAMPLE_TEXTURE2D( _GradientTexture, sampler_GradientTexture, temp_cast_6 ) * _LightTint * input.ase_color );
				float temp_output_8_0_g2090 = lPosterize4215;
				...
				float3 pos338_g2088 = POSITION653;
				float rad280_g2088 = ( SCALE3835 * 0.5 );
				float temp_output_298_0_g2088 = ( 1.0 - ( abs( (pos338_g2088).y ) / rad280_g2088 ) );
				float3 normalizeResult278_g2088 = normalize( ( pos338_g2088 - _WorldSpaceCameraPos ) );
				float4x4 invertVal5_g2089 = Inverse4x4( unity_ObjectToWorld );
				float dotResult283_g2088 = dot( normalizeResult278_g2088 , -mul( unity_ObjectToWorld, float4( (transpose( mul( invertVal5_g2089, UNITY_MATRIX_I_V ) )[2]).xyz , 0.0 ) ).xyz );
				float temp_output_310_0_g2088 = ( temp_output_298_0_g2088 * dotResult283_g2088 );
				float4 worldToScreen279_g2088 = ComputeScreenPos( TransformWorldToHClip( ( pos338_g2088 * float3(1,0.1,1) ) ) );
				float3 worldToScreen279_g2088NDC = worldToScreen279_g2088.xyz/worldToScreen279_g2088.w;
				float2 temp_output_344_0_g2088 = (( float4( worldToScreen279_g2088NDC , 0.0 ) - ase_positionSSNorm )).xy;
				float2 temp_output_384_0_g2088 = ( temp_output_310_0_g2088 * temp_output_344_0_g2088 );
				float2 lightDirScreen483_g2088 = temp_output_384_0_g2088;
				float temp_output_476_0_g2088 = ( _ShadowThreshold * 0.01 );
				float threshold483_g2088 = temp_output_476_0_g2088;
				float stepsSpace483_g2088 = 1.0;
				#if defined( _SCREENSHADOWS_OFF )
				float staticSwitch296_g2088 = -1.0;
				#elif defined( _SCREENSHADOWS_LOW )
				float staticSwitch296_g2088 = 16.0;
				#elif defined( _SCREENSHADOWS_MEDIUM )
				float staticSwitch296_g2088 = 32.0;
				#elif defined( _SCREENSHADOWS_HIGH )
				float staticSwitch296_g2088 = 64.0;
				#elif defined( _SCREENSHADOWS_INSANE )
				float staticSwitch296_g2088 = 128.0;
				#else
				float staticSwitch296_g2088 = -1.0;
				#endif
				float StepQual300_g2088 = staticSwitch296_g2088;
				float stepsNum483_g2088 = StepQual300_g2088;
				float radius483_g2088 = rad280_g2088;
				float vertexToFrag49_g2085 = input.ase_texcoord6.w;
				float vertexToFrag9_g2084 = input.ase_texcoord7.x;
				#ifdef _DAYFADING_ON
				float staticSwitch11_g2084 = vertexToFrag9_g2084;
				#else
				float staticSwitch11_g2084 = 1.0;
				#endif
				float DistanceFade3571 = ( (( DistanceFade )?( vertexToFrag49_g2085 ):( 1.0 )) * staticSwitch11_g2084 );
				float temp_output_335_0_g2088 = DistanceFade3571;
				float mask483_g2088 = temp_output_335_0_g2088;
				float3 worldPos353_g2088 = ReconstructedPos539;
				float3 wPos483_g2088 = worldPos353_g2088;
				float3 lightPos483_g2088 = pos338_g2088;
				float3 camPos483_g2088 = _WorldSpaceCameraPos;
				float2 screenPos483_g2088 = ase_positionSSNorm.xy;
				float3 offsetDir485_g2088 = ( dotResult283_g2088 * normalizeResult278_g2088 );
				float3 offsetDir483_g2088 = offsetDir485_g2088;
				float localExperimentalScreenShadowsURP483_g2088 = ExperimentalScreenShadowsURP( lightDirScreen483_g2088 , threshold483_g2088 , stepsSpace483_g2088 , stepsNum483_g2088 , radius483_g2088 , mask483_g2088 , wPos483_g2088 , lightPos483_g2088 , camPos483_g2088 , screenPos483_g2088 , offsetDir483_g2088 );
				#if defined( _SCREENSHADOWS_OFF )
				float staticSwitch341_g2088 = 1.0;
				#elif defined( _SCREENSHADOWS_LOW )
				float staticSwitch341_g2088 = localExperimentalScreenShadowsURP483_g2088;
				#elif defined( _SCREENSHADOWS_MEDIUM )
				float staticSwitch341_g2088 = localExperimentalScreenShadowsURP483_g2088;
				#elif defined( _SCREENSHADOWS_HIGH )
				float staticSwitch341_g2088 = localExperimentalScreenShadowsURP483_g2088;
				...
				float staticSwitch341_g2088 = 1.0;
				#endif
				float ScreenSpaceShadows1881 = staticSwitch341_g2088;
				float smoothstepResult4598 = smoothstep( 0.0 , _ShadingSoftness , saturate( ( dotResult436 * noise3969 * ScreenSpaceShadows1881 ) ));
				float temp_output_9_0_g2090 = smoothstepResult4598;
				float temp_output_5_0_g2090 = ( 256.0 / temp_output_8_0_g2090 );
				float ShadingMask552 = saturate( ( ( temp_output_8_0_g2090 <= 0.0 ? temp_output_9_0_g2090 : saturate( ( floor( ( temp_output_9_0_g2090 * temp_output_5_0_g2090 ) ) / temp_output_5_0_g2090 ) ) ) + _ShadingBlend ) );
				...
				float3 temp_output_12_0_g2097 = ( ( (temp_output_2_0_g2092).rgb * ( (temp_output_2_0_g2092).a * FinalLightMask4298 * 0.1 ) ) + Spec4287 );
				float4 fetchOpaqueVal5_g2097 = float4( SHADERGRAPH_SAMPLE_SCENE_COLOR( ScreenPos4516.xy ), 1.0 );
				float3 saferPower9_g2097 = abs( (fetchOpaqueVal5_g2097).rgb );
				float3 temp_cast_11 = (saturate( (0.8 + (GradientMask555 - 0.0) * (0.0 - 0.8) / (2.0 - 0.0)) )).xxx;
				#ifdef _ACCURATECOLORS_ON
				float3 staticSwitch11_g2097 = ( ( temp_output_12_0_g2097 * 4 ) * pow( saferPower9_g2097 , temp_cast_11 ) );
				#else
				float3 staticSwitch11_g2097 = temp_output_12_0_g2097;
				#endif
				float3 temp_cast_12 = (0.0).xxx;
				float vertexToFrag32_g2093 = input.ase_texcoord7.y;
				float3 pos75_g2093 = POSITION653;
				float3 pos83_g2093 = pos75_g2093;
				float2 localWorldToScreen83_g2093 = WorldToScreen83_g2093( pos83_g2093 );
				float2 appendResult15_g2093 = (float2(( _ScreenParams.x / _ScreenParams.y ) , 1.0));
				float vertexToFrag15_g2096 = input.ase_texcoord7.z;
				float smoothstepResult33_g2093 = smoothstep( 0.0 , vertexToFrag32_g2093 , length( ( ( ( (localWorldToScreen83_g2093).xy - ScreenPos4516 ) * appendResult15_g2093 ) * vertexToFrag15_g2096 ) ));
				float vertexToFrag36_g2093 = input.ase_texcoord7.w;
				float HaloMask38_g2093 = ( ( 1.0 - smoothstepResult33_g2093 ) * vertexToFrag36_g2093 );
				float temp_output_8_0_g2094 = _HaloPosterize;
				float temp_output_9_0_g2094 = HaloMask38_g2093;
				float temp_output_5_0_g2094 = ( 256.0 / temp_output_8_0_g2094 );
				float HaloPosterized43_g2093 = ( HaloMask38_g2093 * ( temp_output_8_0_g2094 <= 0.0 ? temp_output_9_0_g2094 : saturate( ( floor( ( temp_output_9_0_g2094 * temp_output_5_0_g2094 ) ) / temp_output_5_0_g2094 ) ) ) );
				float2 temp_cast_13 = (( 1.0 - HaloPosterized43_g2093 )).xx;
				float4 temp_output_2_0_g2095 = ( SAMPLE_TEXTURE2D( _GradientTexture, sampler_GradientTexture, temp_cast_13 ) * _HaloTint * input.ase_color );
				float vertexToFrag62_g2093 = input.ase_texcoord8.x;
				float HaloPenetrationMask68_g2093 = saturate( pow( saturate( ( distance( ReconstructedPos539 , _WorldSpaceCameraPos ) - vertexToFrag62_g2093 ) ) , _HaloDepthFade ) );
				#ifdef ___HALO____ON
				float3 staticSwitch54_g2093 = ( (temp_output_2_0_g2095).rgb * ( (temp_output_2_0_g2095).a * HaloMask38_g2093 * HaloPenetrationMask68_g2093 * HaloPosterized43_g2093 ) );
				#else
				float3 staticSwitch54_g2093 = temp_cast_12;
				#endif
				float3 halo4548 = staticSwitch54_g2093;
				float3 vertexToFrag67_g1436 = input.ase_texcoord8.yzw;
				float3 FlickerHue1892 = vertexToFrag67_g1436;
				float3 temp_output_17_0_g2098 = ( ( ( staticSwitch11_g2097 + halo4548 ) * FlickerHue1892 ) * ( DistanceFade3571 * FlickerAlpha416 ) );
				float dither10_g2098 = Dither4x4Bayer( fmod( ase_positionSS_Pixel.x, 4 ), fmod( ase_positionSS_Pixel.y, 4 ) );
				float3 break3_g2098 = temp_output_17_0_g2098;
				float smoothstepResult9_g2098 = smoothstep( 0.0 , _DitherIntensity , ( 0.333 * ( break3_g2098.x + break3_g2098.y + break3_g2098.z ) * ( _DitherIntensity + 1.0 ) ));
				dither10_g2098 = step( dither10_g2098, saturate( smoothstepResult9_g2098 * 1.00001 ) );
				#ifdef _DITHERINGPATTERN_ON
				float3 staticSwitch12_g2098 = ( temp_output_17_0_g2098 * dither10_g2098 );
				#else
				float3 staticSwitch12_g2098 = temp_output_17_0_g2098;
				#endif
				float3 temp_output_4553_0 = staticSwitch12_g2098;
...
Node;AmplifyShaderEditor.RangedFloatNode;1914;-3302.18,-986.6736;Inherit;False;Property;_RandomOffset;RandomOffset;53;1;[HideInInspector];Create;True;0;0;0;False;0;False;0;0;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.CommentaryNode;480;-1606.75,-1080.458;Inherit;False;1475.525;429.7961;;13;4867;4874;3954;262;55;3966;4107;3836;3708;3819;478;539;654;World SphericalMask;0.9034846,0.5330188,1,1;0;0
...
Node;AmplifyShaderEditor.FunctionNode;4867;-1603.651,-1016.364;Inherit;False;Reconstruct World Pos from Depth VR;-1;;2080;474d2b03c8647914986393f8dfbd9fe4;0;0;1;FLOAT4;0
...
Node;AmplifyShaderEditor.ComponentMaskNode;4874;-1276.664,-1016.913;Inherit;False;True;True;True;False;1;0;FLOAT4;0,0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.FunctionNode;4569;-3723.431,239.1101;Inherit;False;DayAlpha;46;;2084;bc1f8ebe2e26696419e0099f8a3e27dc;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;4792;-3905.166,46.45875;Inherit;False;AdvancedCameraFade;12;;2085;e6e830f789d28b746963801d61c2a1ec;0;6;40;FLOAT;0;False;46;FLOAT;0;False;47;FLOAT;0;False;48;FLOAT;0;False;17;FLOAT3;0,0,0;False;20;FLOAT3;0,0,0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.CommentaryNode;1356;-3153.411,-53.7183;Inherit;False;907.1592;385.8282;;6;1881;3576;3838;3540;3830;4869;Experimental Shadows;1,0.0518868,0.0518868,1;0;0
...
Node;AmplifyShaderEditor.FunctionNode;4869;-2853.486,75.6248;Inherit;False;ExperimentalScreenSpaceShadows;40;;2088;79f826106fc5f154c96059cc1326b755;0;4;337;FLOAT3;0,0,0;False;336;FLOAT3;0,0,0;False;370;FLOAT;0;False;335;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;4619;-3099.628,-428.4288;Inherit;False;3DNoiseMap;25;;2086;2fca756491ec7bf4e9c71d18280c45cc;0;5;257;FLOAT3;0,0,0;False;21;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;60;FLOAT;0;False;229;FLOAT2;0,0;False;2;FLOAT;0;FLOAT;213
...
Node;AmplifyShaderEditor.FunctionNode;4213;-1042.616,-478.2397;Inherit;False;SimplePosterize;-1;;2090;163fbd1f7d6893e4ead4288913aedc26;0;2;9;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;4203;1444.219,-490.2576;Inherit;False;SimplePosterize;-1;;2091;163fbd1f7d6893e4ead4288913aedc26;0;2;9;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;4006;533.7668,-81.23064;Inherit;False;Alpha Split;-1;;2092;07dab7960105b86429ac8eebd729ed6d;0;1;2;COLOR;0,0,0,0;False;2;FLOAT3;0;FLOAT;6
...
Node;AmplifyShaderEditor.FunctionNode;4862;501.7485,-425.7458;Inherit;False;HaloFunction;6;;2093;739bbcda129bbae47870a33d01fe91b1;0;5;69;FLOAT3;0,0,0;False;70;FLOAT3;0,0,0;False;72;FLOAT2;0,0;False;74;FLOAT;0;False;80;SAMPLER2D;;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.FunctionNode;4793;1403.63,-78.38047;Inherit;False;AccurateColors;44;;2097;570575a1eb6bdc7409ed58545512a33b;0;3;12;FLOAT3;0,0,0;False;13;FLOAT2;0,0;False;15;FLOAT;0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.FunctionNode;4553;2131.007,-79.12321;Inherit;False;Dithering;36;;2098;b490ae982132eea449373f63bf44f108;0;1;17;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;4905;3491.131,-86.27557;Inherit;False;FinalColor;-1;True;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.CommentaryNode;4893;2796.608,-14.7986;Inherit;False;648.5369;595.7183;Dev Debugging;10;4903;4902;4901;4900;4899;4898;4897;4896;4895;4894;;1,0.3537736,0.3537736,1;0;0
...
Node;AmplifyShaderEditor.GetLocalVarNode;4894;2846.608,35.20142;Inherit;False;539;ReconstructedPos;1;0;OBJECT;;False;1;FLOAT3;0
Node;AmplifyShaderEditor.GetLocalVarNode;4895;2851.074,104.4613;Inherit;False;1927;worldNormals;1;0;OBJECT;;False;1;FLOAT3;0
Node;AmplifyShaderEditor.GetLocalVarNode;4896;2854.14,170.9197;Inherit;False;552;ShadingMask;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;4897;2886.401,236.2428;Inherit;False;3969;noise;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;4898;2825.922,299.4188;Inherit;False;1892;FlickerHue;1;0;OBJECT;;False;1;FLOAT3;0
Node;AmplifyShaderEditor.GetLocalVarNode;4899;2828.27,376.9095;Inherit;False;416;FlickerAlpha;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;4900;2945.893,433.87;Inherit;False;4287;Spec;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;4901;2886.725,498.5837;Inherit;False;1881;ScreenSpaceShadows;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;4902;3143.144,21.95833;Inherit;False;Property;_DEBUGMODE;DEBUG MODE /!\;51;0;Create;True;0;0;0;False;1;Space(5);False;0;0;0;True;;KeywordEnum;8;OFF;ReconstructedPosition;DepthNormals;ShadowMask;Noise;Flicker;Specular;ScreenSpaceShadows;Create;True;True;Fragment;9;1;FLOAT3;0,0,0;False;0;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT3;0,0,0;False;5;FLOAT3;0,0,0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;4903;3001.27,320.9095;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT;0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.RangedFloatNode;4617;1914.272,-839.8246;Inherit;False;Property;_DstBlend;DstBlend;52;2;[HideInInspector];[IntRange];Create;True;0;3;Default;0;Off;1;On;2;1;UnityEngine.Rendering.BlendMode;True;0;False;1;1;0;12;0;1;FLOAT;0
...
WireConnection;4869;337;3830;0
WireConnection;4869;336;3540;0
WireConnection;4869;370;3838;0
WireConnection;4869;335;3576;0
...
ASEEND*/
//CHKSM=611D9D093A4719086EFBDC5F8A67D892C68FF0C9